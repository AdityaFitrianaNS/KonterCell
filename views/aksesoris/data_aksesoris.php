...
$jumlah_data = count(query("SELECT * FROM tb_aksesoris"));
...
// Urutkan berdasarkan huruf(abjad) depan nama aksesoris dari yang terkecil
// Lalu, batasi jumlah data yang ditampilkan maksimal 5.
$query = query("SELECT * FROM tb_aksesoris ORDER BY nama_aksesoris ASC LIMIT $awal_data, $jumlah_per_halaman");

// jika tombol cari ditekan
if (isset($_POST["cari"])) {
    // maka jalankan fungsi cari_aksesoris
    $query = cari_aksesoris($_POST["keyword"]);
}
...
<body>
    ...
    <div class="container-md">
        <h2 class="judul" style="text-align: center;">Data Aksesoris HP</h2>
        <br>
        <!-- Form & button cari -->
        <div class="mt-4 offset-sm-0 col-sm-3">
            ...
            <div class="py-2">
                <a href="tambah_aksesoris" class="btn btn-md btn-primary btn-outline-info text-white">
            ...
            <table class="table table-info table-striped table-bordered border-primary" style="text-align: center; vertical-align: middle;">
                ...
                ?>
                    <tr>
                        ...
                        <td><?= $row['jenis_aksesoris']; ?></td>
                        <td><?= $row['stok']; ?></td>
                        <td>Rp. <?= $row['harga_asli']; ?></td>
                        <td>Rp. <?= $row['harga_jual']; ?></td>
                        <td><?= $tanggal_perbarui->format('d-m-Y'); ?></td>
                        <td>
                            <a href="ubah_aksesoris?id_aksesoris=<?= $row['id_aksesoris']; ?>" class="btn btn-sm btn-primary btn-outline-info btn-table">
                                <i class="bi bi-pencil-square text-white"> Ubah</i>
                            </a>
                            <a href="hapus_aksesoris?id_aksesoris=<?= $row["id_aksesoris"]; ?>" class="btn btn-sm btn-primary btn-outline-info btn-table" id="btn-hapus">
